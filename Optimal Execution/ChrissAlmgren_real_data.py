...
# Real Price Data
real_price = np.random.normal(500, 20, 100000)
...
#             self.k += 1
            ...
            info.price = real_price[k]
            # info.price = self.prevImpactedPrice + np.sqrt(self.singleStepVariance * self.tau) * random.normalvariate(0, 1)
            
#             sigmaSq = self.alphas[0]        
#             for j in range(1, len(self.alphas)):
#                 sigmaSq += self.alphas[j] * (self.a_deque[-j] ** 2)
#             sigmaSq = min(sigmaSq, 200)
#             new_a = np.sqrt(sigmaSq) * random.normalvariate(0, 1)
#             self.a_deque.append(new_a)
#             self.a_deque.popleft()
#             new_return = new_a
#             for j in range(len(self.lagCoeffs)):
#                 new_return += self.lagCoeffs[j] * self.returns_deque[-j-1]
#             self.returns_deque.append(new_return)
#             self.returns_deque.popleft()
#             info.price = self.prevImpactedPrice * np.exp(0.0001 * new_return)
...
#             reward = 1.0 - abs((sharesToSellNow - self.ac_trade_list[self.k]) / self.ac_trade_list[self.k])
...
#             info.share_to_sell_now = sharesToSellNow
...
#                 # Calculate the expected variance according to equation (5) of the AC paper
#                 info.expected_variance = self.singleStepVariance * self.tau * self.totalSRSQ
                
#                 # Calculate the utility
#                 info.utility = info.expected_shortfall + self.llambda * info.expected_variance
                
#                 # Calculate the AC Utility
#                 ac_utility = self.compute_AC_utility(self.total_shares)
        
                # Give a reward
#                 reward = (2 ** (1.0 - ((info.utility - ac_utility) / ac_utility))) / (self.timeHorizon + 1)
#                 print('reward',reward)
#                 reward = ac_utility / info.utility
#                 reward = -(info.utility / ac_utility) + 1.0
...
#             self.prevImpactedPrice -= 0.00003
            reward = 0.0
